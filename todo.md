...
- [x] Prepare the latest validated learner-visibility source for Vercel publishing and provide the Management UI Publish action; the user connected Git and the Git webhook deployed the source.

### Vercel synchronization diagnosis
- [x] Inspect Vercel project/repository linkage, latest deployment commit, and deployment error state.
- [x] Compare the linked Vercel project configuration with public GitHub `main` and identify why commit `6bd239b` or later is not deploying.
