...
import { BookOpen, CheckCircle2, ChevronRight, CircleHelp, Command, GraduationCap, Library, Menu, Search, Sparkles, X } from "lucide-react";
import { useMemo, useState } from "react";
import { Link } from "wouter";
import { toast } from "sonner";

function LearningProgress({ completedIds }: { completedIds: Set<number> }) {
  const overall = Math.round((completedIds.size / allChapters.length) * 100);
  return (
...
function LandingPage({ onStart, completedIds }: { onStart: () => void; completedIds: Set<number> }) {
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f7f8f6] text-slate-950">
      ...
      <main>
        <section className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-16 pt-16 sm:px-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:pb-24 lg:pt-24">
          <div className="relative z-10"><div className="inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-800"><Sparkles className="h-3.5 w-3.5" /> မြန်မာဘာသာဖြင့် အဆင့်လိုက်သင်ယူရန်</div><h1 className="mt-6 max-w-3xl text-4xl font-extrabold leading-[1.14] tracking-tight sm:text-5xl lg:text-6xl">Dart နှင့် Flutter ကို<br /><span className="text-teal-700">လက်တွေ့တည်ဆောက်ရင်း</span><br />နက်နက်ရှိုင်းရှိုင်းသင်ယူပါ</h1><p className="mt-6 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg">အခန်း ၂၀၊ အပိုင်း ၆ ပိုင်းဖြင့် Dart အခြေခံမှ Supabase, architecture, testing နှင့် app deployment အထိ မြန်မာဘာသာဖြင့် code တစ်ကြောင်းချင်းနားလည်အောင်လေ့လာနိုင်သော masterclass ဖြစ်ပါသည်။</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={onStart} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-teal-700 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-teal-700/20 transition hover:bg-teal-800 active:scale-[.98]">သင်ခန်းစာစတင်မည် <ChevronRight className="h-4 w-4" /></button><Link href="/glossary" className="inline-flex min-h-12 items-center gap-2 rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:border-teal-600 hover:text-teal-800"><Library className="h-4 w-4" /> ဝေါဟာရကြည့်မည်</Link></div><div className="mt-9 flex flex-wrap gap-6 text-sm text-slate-600"><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-teal-700" /> Code lab တိုင်းပါဝင်သည်</span><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-teal-700" /> လက်တွေ့စိန်ခေါ်မှုများ</span></div></div>
          <div className="relative rounded-[2rem] border border-slate-200 bg-white p-5 shadow-2xl shadow-slate-900/10 sm:p-7"><div className="absolute -right-5 -top-5 hidden h-28 w-28 rounded-full bg-teal-200/70 blur-2xl sm:block" /><div className="relative flex items-center justify-between border-b border-slate-100 pb-5"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100 text-amber-700"><GraduationCap className="h-5 w-5" /></span><div><p className="text-sm font-bold">သင်ယူမှုလမ်းကြောင်း</p><p className="text-xs text-slate-500">အစပြုသူမှ production အထိ</p></div></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">20 အခန်း</span></div><div className="mt-5 space-y-3">{courseParts.slice(0, 4).map((part, index) => <div key={part.id} className="flex items-center gap-4 rounded-xl bg-slate-50 p-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-sm font-bold text-teal-700 shadow-sm">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-800">{part.title}</p><p className="mt-0.5 text-xs text-slate-500">{part.chapters.length} အခန်း</p></div><div className="h-1.5 w-14 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-teal-600" style={{ width: `${part.chapters.filter((chapter) => completedIds.has(chapter.id)).length / part.chapters.length * 100}%` }} /></div></div>)}</div><button onClick={onStart} className="mt-6 flex w-full items-center justify-between rounded-xl bg-slate-950 px-4 py-3.5 text-left text-sm font-bold text-white"><span>အခန်း ၁ — Dart နှင့် Flutter ကိုမိတ်ဆက်ခြင်း</span><ChevronRight className="h-4 w-4" /></button></div>
        </section>

        <section className="border-y border-slate-200 bg-white"><div className="mx-auto max-w-7xl px-5 py-16 sm:px-8"><div className="max-w-2xl"><p className="text-xs font-bold tracking-[0.16em] text-teal-700">အခန်းစဉ်</p><h2 className="mt-3 text-3xl font-extrabold tracking-tight">အခန်းစဉ်များကို ရှင်းလင်းစွာစီစဉ်ထားပါသည်</h2><p className="mt-4 leading-7 text-slate-600">အပိုင်းတစ်ခုစီသည်အရင်အပိုင်းပေါ်အခြေခံထားသောကြောင့် စတင်သူတစ်ဦးလည်းလမ်းမပျောက်ဘဲလိုက်နိုင်ပါသည်။</p></div><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{courseParts.map((part, index) => <button key={part.id} onClick={onStart} className="group rounded-2xl border border-slate-200 p-5 text-left transition hover:-translate-y-0.5 hover:border-teal-400 hover:shadow-lg hover:shadow-teal-950/5"><div className="flex items-start justify-between gap-4"><span className="text-sm font-extrabold text-teal-700">အပိုင်း {index + 1}</span><ChevronRight className="h-4 w-4 text-slate-300 transition group-hover:text-teal-700" /></div><h3 className="mt-6 text-lg font-bold">{part.title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{part.description}</p><p className="mt-5 text-xs font-bold text-slate-500">{part.chapters.length} အခန်း</p></button>)}</div></div></section>
      </main>
    </div>
  );
}
...
export default function Home() {
  ...
  const saveProgress = trpc.progress.setCompleted.useMutation({
    onSuccess: () => utils.progress.list.invalidate(),
  ...
  const activePart = courseParts.find((part) => part.id === activeChapter.partId) ?? courseParts[0];
  ...
  if (!isLearning) return <LandingPage onStart={() => setIsLearning(true)} completedIds={completedIds} />;

  return (
    <div className="min-h-screen bg-[#f7f8f6] text-slate-950">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="flex h-16 items-center gap-3 px-4 sm:px-6"><button onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 lg:hidden" aria-label="သင်ခန်းစာမီနူးဖွင့်ရန်"><Menu className="h-5 w-5" /></button><button onClick={() => setIsLearning(false)} className="flex min-w-0 items-center gap-2 text-left"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-950 text-teal-300"><BookOpen className="h-4 w-4" /></span><span className="hidden min-w-0 sm:block"><strong className="block truncate text-sm">Dart & Flutter Masterclass</strong><span className="block text-[11px] text-slate-500">မြန်မာဘာသာသင်ခန်းစာ</span></span></button><div className="ml-auto flex items-center gap-2"><button onClick={() => setSearchOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:border-teal-600 hover:text-teal-800"><Search className="h-4 w-4" /><span className="hidden sm:inline">စာအုပ်တစ်လျှောက်ရှာရန်</span><Command className="hidden h-3.5 w-3.5 text-slate-400 lg:block" /></button><Link href="/glossary" className="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100" aria-label="ဝေါဟာရစာအုပ်"><CircleHelp className="h-5 w-5" /></Link>{isAuthenticated ? <span className="hidden rounded-full bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-800 sm:inline">{user?.name ?? "သင်ယူသူ"}</span> : <button onClick={startLogin} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-bold text-white">ဝင်ရောက်မည်</button>}</div></div></header>

      <div className="flex min-h-[calc(100vh-64px)]"><div className="hidden shrink-0 lg:block"><LessonSidebar parts={courseParts} activeChapterId={activeChapterId} completedIds={completedIds} onSelect={selectChapter} /></div><main className="min-w-0 flex-1"><LessonReader chapter={activeChapter} part={activePart} completed={completedIds.has(activeChapter.id)} onToggleCompletion={handleCompletion} isSaving={saveProgress.isPending || authLoading} isAuthenticated={isAuthenticated} /></main><aside className="hidden w-[312px] shrink-0 border-l border-slate-200 bg-white p-5 xl:block"><LearningProgress completedIds={completedIds} /><div className="mt-5 rounded-2xl bg-slate-950 p-5 text-white"><p className="text-sm font-bold">သင်ယူမှုအကြံပြုချက်</p><p className="mt-2 text-sm leading-6 text-slate-300">Code ကိုကိုယ်တိုင်run ပြီး challenge အပြီးလုပ်မှ နောက်အခန်းသို့သွားပါ။</p></div></aside></div>

      {sidebarOpen ? <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 bg-slate-950/35" onClick={() => setSidebarOpen(false)} aria-label="မီနူးပိတ်ရန်" /><div className="relative h-full w-[min(88vw,330px)] shadow-2xl"><LessonSidebar parts={courseParts} activeChapterId={activeChapterId} completedIds={completedIds} onSelect={selectChapter} onClose={() => setSidebarOpen(false)} /></div></div> : null}
